...
class PageHome extends PureComponent<Props, State> {
  constructor(props) {
    ...
    this.onAuctionFinishLoadMore = this.onAuctionFinishLoadMore.bind(this);
    this.handleLoadMore = this.handleLoadMore.bind(this);
    this.handleFilter = this.handleFilter.bind(this);
    this.reloadLeftAuctions = this.reloadLeftAuctions.bind(this);
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      nextProps.errorStatus === 400 ||
      nextProps.errorStatus === 500 ||
      nextProps.errorStatus === 404
    )
      history.push('/error');
  }
  ...
  componentWillUnmount() {
    const {
      actions: { leaveHome, resetAuctionStates }
    } = this.props;
    leaveHome();
    resetAuctionStates();
  }
  ...
  reloadLeftAuctions() {
    const {
      actions: { getAuctionsIfNeeded, getAuctionsFinishedIfNeeded },
      pageAuctionsFinished,
      pageNumberAuction,
      filter
    } = this.props;
  ...
  onAuctionFinishLoadMore(e: any) {
    e.preventDefault();
    const {
      actions: { auctionsFinishLoadMore },
      pageAuctionsFinished
    } = this.props;
  ...
  render() {
    const {
      ...
      filterTypeAuction,
      filterName,
      actions: { getAuctionsIfNeededByCategory, resetAuctionStates }
    } = this.props;
            ...
            <div className="wrapper">
              <div className="_banner">
                <div className="container">
                  <div
                    id="banner-slide"
                    className="owl-carousel banner-slide"
                  />
                          ...
                          <div className="box_sidebar">
                            ...
                            <div className="box_sidebar_content">
                              ...
                              <div className="text-center mgt-20">
                                {auctionsFinished.length % 10 === 0 &&
                                auctionsFinished.length >=
                                  pageAuctionsFinished * 10 ? (
                                  <Link
                                    to="#"
                                    className="btn_md btn_green"
                                    onClick={this.onAuctionFinishLoadMore}
                                  >
                                    {t('Load more')}
                                  </Link>
                                ) : (
                      ...
                      <div className="col-xs-12 col-sm-7 col-md-8 col-lg-8">
                        <div className="content_right">
                          ...
                          <div className="content_inner">
                              ...
                              <div className="row">
                                {auctions.map((auc, key) => (
                                  <div className="item" key={key}>
                                    <AuctionCard
                                      title={
                                        auc && auc.product && auc.product.name
                                      }
                                      image={
                                        auc &&
                                        auc.product &&
                                        auc.product.featureImage.thumb
                                      }
                                      numberTotal={
                                        auc &&
                                        auc.product &&
                                        auc.product.chanceNumber
                                      }
                            ...
                            <div className="text-center mgt-20">
                              {auctions.length % 10 === 0 &&
                              auctions.length >= pageNumberAuction * 10 ? (
                                <Link
                                  to="#"
                                  className="btn_md btn_green"

...
class EditProfile extends PureComponent<Props, State> {
    ...
    handlePreviewImage(e) {
        if(e.target.files[0]){
          this.setState({ avatarInput: e.target.files[0], avatarValue:e.target.value });
    ...
    renderField = ({ input, placeholder, type, fieldValue, trans, meta: { touched, error, warning } }) => {
        return (
            <div className="form-group">
                <label className="control-label" htmlFor="nickNameInput">{trans('Nickname')}</label>
                <input
                    {...input}
    ...
    renderFileInputField = ({ input, type, fieldValue, trans, meta: { touched, error, warning } }) => {
        return (
          <div>
            <div className="input-group" style={{margin:"5px 0"}}>
              <span className="input-group-btn">
                <button onClick={this.handlePickImage} className="btn btn-default">{trans('Browse')}</button>
              </span>
              <input
                  {...input}
                  type={type}
    ...
    render() {
      ...
      const {
        nickNameInput, avatarValue, isOK
      } = this.state;
                      ...
                      <div className="help-content" style={{minHeight: "903px"}}>
                        <div>
                          ...
                          <div className="mgt-20">
                            <form>
                              <div className="text-center">
                                  {isError ? <span className="text-danger">{t(errorMessage)}</span>: <span className="text-success">{t(errorMessage)}</span>}
                              </div>
                              <div className="user_box_avatar col-md-offset-4">
                                  <div className="image_preview">
                                    <img src={avatar || `/images/preview.png`} alt={nickname} title={nickname} />
                                    <input type="file" id="avatar-profile" style={{display:"none"}} onChange={this.handlePreviewImage} />
                                  </div>
                              </div>
                              <div>
                              <div className="input-group" style={{margin:"5px 0"}}>
                                <span className="input-group-btn">
                                  <button onClick={this.handlePickImage} className="btn btn-default">{t('Browse')}</button>
                                </span>
                                <input
                                    type="text"
                                    className="form-control"
                              ...
                              />
                              <div className="text-center">
                                <button disabled={isEditting || (isOK && nickname !== nickNameInput)} className="btn btn-green-2 btn-square btn-square-sm" onClick={this.handleUpdateClick}>
                                {
                                    isEditting
                                        ?
                                        <span>

...
class Contact extends PureComponent<Props, State> {
  ...
  renderField = ({ input, placeholder, type, fieldValue, trans, meta: { touched, error, warning } }) => {
      return (
          <div className="form-group">
              <label className="control-label" htmlFor="email">{trans('Email')}</label>
              <input
                  {...input}
  ...
  renderTextAreaField = ({ input, placeholder, rows, fieldValue, trans, meta: { touched, error, warning } }) => {
      return (
          <div className="form-group">
              <label className="control-label" htmlFor="content">{trans('content')}</label>
              <textarea
                  {...input}
                  placeholder={placeholder}
                  rows={rows}
  ...
  renderSelectField = ({ input, contactCategories, fieldValue, trans, meta: { touched, error, warning } }) => {
      return (
          <div className="form-group">
            <label className="control-label" htmlFor="contactCategory">{trans('contact_title')}</label>
            <select
                  {...input}
                  className={`form-control`}
                  id={`contact-category`}
            ...
            >
              ...
              {
                  contactCategories.map((contact, key) => <option key={key} value={contact._id}>{contact.name}</option>)
              }
            </select>
            {touched && ((error && <span className="text-danger">{trans(error)}</span>) ||
                (warning && <span className="text-danger">{trans(warning)}</span>))}
                  ...
                  <div className="help-content">
                    <div>
                        ...
                        <div className="mgt-20">
                            <form>
                                <div className="text-center">
                                    {isError ? <span className="text-danger">{t(messasge)}</span>: <span className="text-success">{t(messasge)}</span>}
                                </div>
                                <Field
                                ...
                                <div className="text-right">
                                  <button className="btn btn-green-2 btn-square btn-square-sm"

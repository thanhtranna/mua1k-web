...
class App extends Component {
  ...
  componentDidMount() {
    const {
      actions: { fetchUserInfoDataIfNeeded }
    } = this.props;
    ...
    return (
      <div>
        <Header
          ...
          searchAuctionByKeyword={searchAuctionByKeyword}
          resetAuctionStates={resetAuctionStates}
          keyword={keyword}
          pageNumberAuction={pageNumberAuction}
          history={history}
          filter={filter}
          act={act}
          uid={uid}
  ...
  handlesLogout = async (event: SyntheticEvent<>) => {
    if (event) {
      event.preventDefault();
    }
    const {
      actions: { disconnectUser, errorBadRequest },
      history
    } = this.props;
    try {
      disconnectUser();
